...
const getColumns = ({
  ...
  userId: string;
  onRemoveUser: (details: { userId: string; workspaceId: string }) => void;
  ...
  },
  {
    ...
    header: "Tags",
    cell: ({ row }) => {
      // This fix ensures the component won't crash if tags are undefined
      ...
      return (
        <div className="text-right">
          <Button
            variant="destructive"
            size="sm"
            onClick={() => onRemoveUser({ userId, workspaceId })}
          >
            Remove
          </Button>
        </div>
...
export function RemoveFromWorkspaceTable({
  workspaces,
  userId,
  onRemoveUser,
}: UserWorkspacesTableProps) {
  const columns = React.useMemo(
    () => getColumns({ userId, onRemoveUser }),
    [userId, onRemoveUser]
  );
